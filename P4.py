...
# ---
# 
# * Estudiante: **Andrés Arias Campos**
# * Carné: **B80661**
# * Grupo: **1**
# 
# ---
...
senal_Tx_BPSK, Pm, portadora, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx_BPSK, Pm, SNR)
...
ax2.plot(senal_Tx_BPSK[0:600], color='g', lw=2) 
ax2.set_ylabel('$s(t)$')
...
# Para la primera parte de la modulación QPSK lo que se hace es modificar las funciones modulador y desmodulador para adaptarlas a la modulación requerida. Para ello se aplica el mismo procedimiento de la modulación BPSK pero ahora aplicado a dos portadoras, entonces las funciones quedan de la siguiente forma:

# In[9]:


def modulador_QPSK(bits, fc, mpp):
    '''Un método que simula el esquema de 
    ...
    moduladora_I = np.zeros(t_simulacion.shape)  
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    Pm1 = (1 / (N*Tc)) * np.trapz(pow(senal_Tx_I, 2), t_simulacion)
    Pm2 = (1 / (N*Tc)) * np.trapz(pow(senal_Tx_Q, 2), t_simulacion)
...
def demodulador_QPSK(senal_Rx, portadora_I, portadora_Q, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
    ...
    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones (se aplica el procedimiento para cada portadora)
        producto_I = senal_Rx[i*mpp : (i+1)*mpp] * portadora_I
        senal_demodulada[i*mpp : (i+1)*mpp] = producto_I
        Ep_I = np.sum(producto_I)
        
        producto_Q = senal_Rx[i*mpp : (i+1)*mpp] * portadora_Q
        senal_demodulada[i*mpp : (i+1)*mpp] = producto_Q
        Ep_Q = np.sum(producto_Q) 
        
        # Criterio de decisión por detección de energía (se disminuyen estos factores para evitar un número alto de errores)
        if Ep_I > Es_I*0.8:
            bits_Rx[i] = 1
        elif Ep_Q > Es_Q*0.8:
            bits_Rx[i] = 1
...
# Ahora se hacen las pruebas para ver el efecto de ambas funciones:
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadora_I, portadora_Q, moduladora = modulador_QPSK(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_QPSK(senal_Rx, portadora_I, portadora_Q, mpp)
...
# Se puede notar que la señal enviada es consistente con lo explicado anteriormente en la explicación de la modulación QPSK. Ahora se prosigue a completar las pruebas de estacionaridad y ergodicidad:
...
N = 4                          # Hay 4 posibilidades de la onda
...
# Se puede observar que la media es constante en todo momento y que la autocorrelación es dependiente en todo momento del diferencial temporal, por esto se puede afirmar que si hay estacionaridad y luego se tendría que el proceso es ergódico dado que las medias estadisticas igualan a las temporales.

# Para el cálculo y la graficación de la densidad de potencia se tendría que:
...
# Se tendría que la mayoría de valores se encuentran alrededor de la frecuencia de las portadoras, que haya más de una portadora no afecta debido a que el número de simbolos se mantiene dado que se usan dos bits por cada simbolo. Lo único que afecta es que se recurre a menos bits para transmitir la señal.
